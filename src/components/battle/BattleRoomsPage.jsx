...
function BattleRoomsPage() {
  ...
  // Restore active battle room on page refresh
  useEffect(() => {
    const savedRoomId = sessionStorage.getItem("active_battle_room_id");
    if (savedRoomId && !currentRoom) {
      axios.get(`${BASE}/battle/rooms/public`).then(res => {
        const found = (res.data?.payload || []).find(r => r.roomId === savedRoomId);
  ...
  function handleRoomCreated(data) {
    if (data?.room?.roomId) {
      sessionStorage.setItem("active_battle_room_id", data.room.roomId);
    }
  ...
  function handleJoinRoom(roomId) {
    ...
    const handleUpdated = ({ room }) => {
      if (room?.roomId === roomId) {
        sessionStorage.setItem("active_battle_room_id", room.roomId);
        setCurrentRoom(room);
        setView("room");
        socket.off("room_updated", handleUpdated);
        socket.off("join_result", handleResult);
      }
    };
    const handleResult = (res) => {
      if (res?.success && res.room) {
        sessionStorage.setItem("active_battle_room_id", res.room.roomId);
    ...
    socket.on("join_result", handleResult);
  }
  ...
  if (view === "room" && currentRoom) {
    return (
      <BattleRoom
        ...
        socket={socket}
        onLeave={() => {
          sessionStorage.removeItem("active_battle_room_id");
          setView("lobby");
          setCurrentRoom(null);
          loadPublicRooms();
        }}
